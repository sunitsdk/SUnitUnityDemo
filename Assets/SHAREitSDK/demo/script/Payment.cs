...
WoSkuMerchantParamBean.Builder builder = new WoSkuMerchantParamBean.Builder()
    .setMerchantId(merchantId)
    ...
    .setShowResult(useMerchantPayResult ? MerchantParamBean.PAY_RESULT_TYPE_MERCHANT : MerchantParamBean.PAY_RESULT_TYPE_SDK);
        ...
        WoSkuMerchantParamBean woSkuMerchantParamBean = builder.build();


        shareitSDK.recharge(woSkuMerchantParamBean, new SHAREitSDK.RechargeListener.onRechargeResponseCallback((int code, string merchantOrderNo, string payMessage, string payExtra) =>
        {
            string result = "code=" + code + " merchantOrderNo=" + merchantOrderNo + " message=" + payMessage + " extra=" + payExtra;
            Debug.Log(TAG + " onRecharge result " + result);
            if (code == 10000)
    ...
    private void getProductList()
    {
        ...
        shareitSDK.queryProducts(bean, productIds, new SHAREitSDK.RechargeListener.OnProductResponseCallback((int code, string message, List<ProductDetailBean> productDetailList) =>
        {
            if (code == 10000)
            {
    ...
    private void onProductItemClick(ProductDetailBean bean)
    {
        MerchantParamBean merchantParam = new MerchantParamBean.Builder()
            ...
            .build();
        shareitSDK.launchBillingFlow(merchantParam, new SHAREitSDK.RechargeListener.onRechargeResponseCallback((int code, string merchantOrderNo, string message, string reference) =>
        {
